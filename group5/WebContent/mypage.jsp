				...
				<tbody>
					<c:forEach items="${rsvList}" var="rsvList">
						<tr>
							...
							<td><a
								href="./rc?nextSt=cancel&scheduleNum=${rsvList.scheduleNum}"
								onclick="return confirm('정말 예매를 취소하시겠습니까?')"
								class="btn btn-inverse">취소하기</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
						...
						<td><c:choose>
								<c:when test="${qnaList.askStatus == 0 }">
									<a href="./qc?nextSt=qDetail&qnaNum=${qnaList.qnaNum }"
										...
										style="color: #181a1c !important"> ${qnaList.title }</a>
								</c:when>
